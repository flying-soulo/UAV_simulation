...
class Mixer:
    ...
    def run(self, mode: str, input: Controls_class)-> Actuator_class:
        self.input = input
        match (mode):
            case "Auto":
                self.output.FW.throttle = linear_scale(
                    self.input.FW.throttle,
                    in_min=configs.stick_input_range[0],
                    in_max=configs.stick_input_range[1],
                    out_min=configs.PWM_output_range[0],
                    out_max=configs.PWM_output_range[1],
                )
