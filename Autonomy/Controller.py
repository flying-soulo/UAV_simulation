...
class MasterController:
    ...
    def __init__(self):
        ...
        self.dt = 0.01  # Default time step for PID updates

    # Quadrotor control methods
    def run_x_position(self, target_x, current_x, current_x_vel):
        target_x_vel = self.quad_x_pos.run_pid(target_x, current_x, self.dt)
        target_x_accel = self.quad_x_vel.run_pid(target_x_vel, current_x_vel, self.dt)
        return target_x_accel

    def run_y_position(self, target_y, current_y, current_y_vel):
        target_y_vel = self.quad_y_pos.run_pid(target_y, current_y, self.dt)
        target_y_accel = self.quad_y_vel.run_pid(target_y_vel, current_y_vel, self.dt)
        return target_y_accel

    def run_altitude(self, target_z, current_z, current_z_vel, current_z_accel):
        target_z_vel = self.quad_z_pos.run_pid(target_z, current_z, self.dt)
        target_z_accel = self.quad_z_vel.run_pid(target_z_vel, current_z_vel, self.dt)
        target_z_jerk = self.quad_z_accel.run_pid(
            target_z_accel, current_z_accel, self.dt
        )
        return target_z_jerk

    def run_phi(self, target_phi, current_phi, current_phi_rate):
        target_phi_rate = self.quad_phi.run_pid(target_phi, current_phi, self.dt)
        target_phi_accel = self.quad_phi_rate.run_pid(
            target_phi_rate, current_phi_rate, self.dt
        )
        return target_phi_accel

    def run_theta(self, target_theta, current_theta, current_theta_rate):
        target_theta_rate = self.quad_theta.run_pid(
            target_theta, current_theta, self.dt
        )
        target_theta_accel = self.quad_theta_rate.run_pid(
            target_theta_rate, current_theta_rate, self.dt
        )
        return target_theta_accel

    def run_psi(self, target_psi, current_psi, current_psi_rate):
        target_psi_rate = self.quad_psi.run_pid(target_psi, current_psi, self.dt)
        target_psi_accel = self.quad_psi_rate.run_pid(
            target_psi_rate, current_psi_rate, self.dt
        )
        return target_psi_accel

    def run_quad_mode(
        self, current_state, target_x, target_y, target_z, target_heading, dt
    ):
        ...
        roll_throttle = self.run_phi(
            target_phi, current_attitude[0], current_rates[0], dt
        )

        # Compute pitch throttle from theta controller
        pitch_throttle = self.run_theta(
            target_theta, current_attitude[1], current_rates[1], dt
        )

        # Compute yaw throttle from psi controller
        yaw_throttle = self.run_psi(
            target_heading, current_attitude[2], current_rates[2], dt
        )
        # Compute base throttle from z position controller
        throttle_base = self.run_altitude(
            target_z, current_z, current_z_vel, current_z_accel, dt
        )
        ...
        self.throttle[0] = (
            throttle_base - roll_throttle - pitch_throttle - yaw_throttle
        )  # Left Front (CCW)
        self.throttle[1] = (
            throttle_base + roll_throttle - pitch_throttle + yaw_throttle
        )  # Right Front (CW)
    ...
    def throttle_controller(self, target_airspeed, current_airspeed, dt):
        throttle_command = self.fw_airspeed.run_pid(
            target_airspeed, current_airspeed, dt
        )
        return throttle_command

    def elevator_controller(
        self, target_altitude, current_altitude, current_pitch, current_pitch_rate, dt
    ):
        ...
        elevator_deflection = self.fw_pitch_rate.run_pid(
            pitch_rate_command, current_pitch_rate, dt
        )
        elevator_deflection = -elevator_deflection  # elevator down is positive
        return elevator_deflection

    def aileron_controller(
        self, target_heading, current_heading, current_roll, current_roll_rate, dt
    ):
        ...
        aileron_deflection = self.fw_roll_rate.run_pid(
            roll_rate_command, current_roll_rate, dt
        )
    ...
    def run_fixed_wing_mode(
        self, current_state, target_airspeed, target_heading, target_altitude, dt
    ):
        ...
        current_rates = current_state[
            6:9
        ]  # Angular rates (roll_rate, pitch_rate, yaw_rate)

        # Compute throttle command using throttle_controller
        throttle_command = self.throttle_controller(
            target_airspeed, current_airspeed, dt
        )

        # Compute aileron deflection using aileron_controller
        aileron_deflection = self.aileron_controller(
            target_heading, current_heading, current_attitude[0], current_rates[0], dt
        )

        # Compute elevator deflection using elevator_controller
        elevator_deflection = self.elevator_controller(
            target_altitude, current_altitude, current_attitude[1], current_rates[1], dt
        )
        ...
        return throttle_command, np.array(
            [aileron_deflection, elevator_deflection, rudder_deflection]
        )
