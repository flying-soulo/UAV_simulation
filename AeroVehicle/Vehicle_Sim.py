...
class Aero_Vehicle_sim:
    ...
    def simulate_one_step(self, input_state, motor_thrust, ctrl_srfc_deflection, dt):
        ...
        min_deflection, max_deflection = np.radians(-20), np.radians(
            20
        )  # Degrees for control surfaces
        ...
        ctrl_srfc_deflection = [
            wrap(value, min_deflection, max_deflection)
            for value in ctrl_srfc_deflection
        ]

        acc_body, omega_dot, forces_moments = six_DOF_motion(
            self.vehicle_prop, self.state, motor_thrust, ctrl_srfc_deflection
        )
        ...
        V_ned = (
            R_body_to_ned @ self.state[3:6]
        )  # [u, v, w] in body frame to [Vx, Vy, Vz] in NED frame
        ...
        self.state[6] = wrap(
            self.state[6] + self.state[9] * dt, -np.pi / 2, np.pi / 2
        )  # phi (roll angle)
        self.state[7] = wrap(
            self.state[7] + self.state[10] * dt, -np.pi / 2, np.pi / 2
        )  # theta (pitch angle)
        self.state[8] = wrap(
            self.state[8] + self.state[11] * dt, -np.pi, np.pi
        )  # psi (yaw angle)
